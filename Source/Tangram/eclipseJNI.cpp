...
jstring tangram_extend(JNIEnv * env, jobject obj, jstring key, jstring data, jstring features) {
	...
	if (g_pTangram)
	{
		...
		strRet = g_pTangram->tangram_for_eclipse(strKey, strData, strFeatures);
	}
	if (_features != NULL) {
...
void registerNatives(JNIEnv *env) {
	...
	jclass bridge = env->FindClass(g_pTangram->m_strBridgeJavaClass);
...
JavaResults * startJavaJNI(_TCHAR* libPath, _TCHAR* vmArgs[], _TCHAR* progArgs[], _TCHAR* jarFile)
{
	...
	for (i = 0; i < numVMArgs; i++) {
		...
		stroption = g_pTangram->ConfigJavaVMInfo(stroption);
		if (stroption == _T(""))
	...
	init_args.version = g_pTangram->m_nJVMVersion;
#endif
	...
	if (createJavaVM(&jvm, &env, &init_args) == 0)
	{
		...
		if (g_pTangram)
		{
			g_pTangram->m_pTangramDelegate->m_pJVM = jvm;
			g_pTangram->m_pTangramDelegate->m_pJVMenv = env;
			if (env)
			{
				g_pTangram->m_pTangramDelegate->systemClass = env->FindClass("java/lang/System");
				try
				{
					if (g_pTangram->m_pTangramDelegate->systemClass != nullptr)
					{
						g_pTangram->m_pTangramDelegate->exitMethod = env->GetStaticMethodID(g_pTangram->m_pTangramDelegate->systemClass, "exit", "(I)V");
						g_pTangram->m_pTangramDelegate->loadMethod = env->GetStaticMethodID(g_pTangram->m_pTangramDelegate->systemClass, "load", "(Ljava/lang/String;)V");
					}
				}
				catch (...)
				{
		...
		registerNatives(env);
		USES_CONVERSION;
		char * mainClassName = W2A(g_pTangram->InitEclipse(jarFile));
