...
void CTangramHtmlTreeWnd::UpdateData(CString strData)
{
	...
	if (_pHostXmlParse->LoadXml(strXml))
	{
		...
		if(g_pTangram->m_pHostDesignUINode)
			g_pTangram->m_pHostDesignUINode->m_pDocXmlParseNode = m_pHostXmlParse;
...
int CTangramHtmlTreeWnd::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
	if (m_pHostWnd)
	{
		g_pTangram->m_pActiveNode = m_pHostWnd->m_pWndNode;
		g_pTangram->m_pCompositor = m_pHostWnd->m_pWndNode->m_pTangramNodeCommonData->m_pCompositor;
	}
	return MA_ACTIVATE;
	...
	if(hItem)
	{
		...
		if (pXTCD->m_bEnabled)
		{
			...
			if (g_pTangram->m_pDocDOMTree == this)
			{
				...
				{
					CWndNode* pRootNode = g_pTangram->m_pDesignWindowNode->m_pRootObj;
					IWndNode* _pNode = NULL;
					long nCount = 0;
					...
					if (_pNode)
					{
						if (g_pTangram->m_pCLRProxy)
							g_pTangram->m_pCLRProxy->SelectNode(_pNode);
					}
				}
			}
...
//=============================================================================
{
	...
	if(pXTCD->m_strTangramXML!=_T("")/*&&m_LasthItem!=hItem*/)
	{
		...
		if(m_Parse.LoadXml(pXTCD->m_strTangramXML))
		{
			...
			if(strProperty!=_T(""))
			{
				if(m_pObj)
				{
					...
					auto it = m_pObj->m_mapTangramTreeViewCallBack.end();
		...
		if(m_pCompositor==NULL)
		{
			...
			//if(m_pCompositor==NULL)
			//{
			//	HRESULT hr = pNode->get_Compositor(&m_pCompositor);
			//	if(hr==S_OK)
	...
	{
		if(m_pCompositor&&m_hPreviousItem)
		{
			//m_pCompositor->Deatch();
			m_bAttached = false;
		}
	}
