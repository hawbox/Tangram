...
int GetLaunchMode()
{
	...
	if (g_pTangram->launchMode == -1)
	{
		...
		g_pTangram->launchMode = determineVM(&msg);
		if (g_pTangram->launchMode == -1) {
			/* problem */
...
static int _run(int argc, _TCHAR * argv[], _TCHAR * vmArgs[])
{
...
#ifdef _WIN32
	if (g_pTangram->launchMode == LAUNCH_JNI && (debug || needConsole)) {
		createConsole();
	}
#endif
	...
	if (!noSplash && showSplashArg)
	{
		...
		if (splashBitmap != NULL && g_pTangram->launchMode == LAUNCH_JNI) {
			showSplash(splashBitmap);
		}
	}

	/* not using JNI launching, need some shared data */
	if (g_pTangram->launchMode == LAUNCH_EXE && createSharedData(&sharedID, MAX_SHARED_LENGTH)) {
		if (debug) {
			if (!suppressErrors)
	...
	if (g_pTangram->launchMode == LAUNCH_JNI) {
	...
	userVMarg = vmArgs;
	getVMCommand(g_pTangram->launchMode, argc, argv, &vmCommandArgs, &progCommandArgs);

	if (g_pTangram->launchMode == LAUNCH_EXE) {
		vmCommand = buildLaunchCommand(javaVM, vmCommandArgs, progCommandArgs);
	}

	int launchMode = g_pTangram->launchMode;
	...
	while (running)
	{
		...
		if (launchMode == LAUNCH_JNI)
		{
			...
			if (g_pTangram->m_pCLRProxy)
				g_pTangram->m_pCLRProxy->TangramAction(_T("<begin_create_jvm_for_eclipse/>"), nullptr);
...
_TCHAR* findStartupJar() {
	...
	/* equinox startup jar? */
	CString strPreFix = g_pTangram->m_strStartJarPath;
					...
					if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, jreKeyName, 0, KEY_READ, &jreKey) == ERROR_SUCCESS) {
						if (RegQueryValueEx(jreKey, _T_ECLIPSE("CurrentVersion"), NULL, NULL, (LPBYTE)& keyName, &length) == ERROR_SUCCESS) {
							...
							if (jniLib != NULL && ::loadLibrary(jniLib) != NULL) {
								...
								{
									g_pTangram->m_nJVMVersion = JNI_VERSION_1_8;
						...
						while (RegEnumKeyEx(jreKey, j++, keyName, &length, 0, 0, 0, 0) == ERROR_SUCCESS) {
							...
							if (_tcsncmp(_T("1.6"), keyName, 3) <= 0) {
								...
								if (jniLib != NULL && ::loadLibrary(jniLib) != NULL) {
									...
									{
									case 16:
										g_pTangram->m_nJVMVersion = JNI_VERSION_1_6;
										break;
									case 14:
										g_pTangram->m_nJVMVersion = JNI_VERSION_1_4;
										break;
									case 12:
										g_pTangram->m_nJVMVersion = JNI_VERSION_1_2;
										break;
									case 11:
										g_pTangram->m_nJVMVersion = JNI_VERSION_1_1;
										break;
									}
