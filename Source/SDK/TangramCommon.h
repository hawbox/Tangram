...
namespace TangramCommon {
	...
	typedef struct tagCompositorInfo
	{
		HWND			m_hCtrlHandle;
		IDispatch*		m_pDisp;
	...
	typedef CTangramImpl*(__stdcall* GetTangramImplFunction)(ITangram** ppTangram);
	...
	public:
		ITangramWindowProvider() { 
			m_strProviderID = _T(""); 
			m_strTangramContainer = _T("");
			m_pCreatingNode = nullptr;
		}
	...
	public:
		...
		virtual BOOL InitTangram(void* pVoid);
		virtual BOOL TangramSaveAllModified();
	...
	public:
		...
		virtual ~ITangramDelegate() {
			if (m_pJVMenv)
			{
				if (systemClass != nullptr && exitMethod != nullptr)
				{
					try
					{
						OutputDebugString(_T("Exit Eclipse JVM from MFC\n"));
	...
	extern CTangramImpl* g_pTangramImpl;
}  // namespace TangramCommon
