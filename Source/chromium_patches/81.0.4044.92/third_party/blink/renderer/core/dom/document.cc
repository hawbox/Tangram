...
Element* Document::CreateRawElement(const QualifiedName& qname,
  ...
  if (qname.NamespaceURI() == html_names::xhtmlNamespaceURI) {
    ...
    if (!element) {
      ...
      // >>>>> TANGRAMDEV
      else if (custom_tag_names_.Contains(qname.LocalName()))
        element = MakeGarbageCollected<HTMLNewElement>(qname, *this);
      // <<<<< TANGRAMDEV
      else
...
// >>>>> TANGRAMDEV
void Document::SendCustomMessage(std::string msg) {
  WebLocalFrameImpl::FromFrame(GetFrame())->SendCustomMessage(msg);
}

void Document::OnCustomMessageReceived(std::string msg) {
  // TODO: Update element attributes
}
// <<<<< TANGRAMDEV
