...
namespace WindowsFormsApp1
{
    static class WebDelegate
    {
        private static int Init(string argument)
        {
            ...
            Tangram.OnTangramCloudMsgReceived += Tangram_OnTangramCloudMsgReceived;
            string strPath = Application.StartupPath + "\\tangramtesthelp.html";
            if (System.IO.File.Exists(strPath))
                Tangram.CreateBrowser(IntPtr.Zero, strPath);
            return 0;
        }

        private static void Tangram_OnTangramCloudMsgReceived(TangramSession cloudSession)
        {
            string strMsgID = cloudSession.GetString("msgID");
            IntPtr nHandle = (IntPtr)cloudSession.GetInt64("nodehandle");
        }
